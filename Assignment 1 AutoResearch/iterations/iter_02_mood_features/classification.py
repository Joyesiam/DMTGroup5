"""
Iteration 2 -- Mood Features Classification.
Full features + mood volatility + interactions.
"""
...
def run():
    print("=" * 60)
    print("ITERATION 2 -- MOOD FEATURES CLASSIFICATION")
    print("=" * 60)

    daily = load_and_clean()

    # Build features WITH volatility and interactions (new in this iteration)
    features_df = build_features(
        daily, window_sizes=[7], n_lags=3,
        include_volatility=True, include_interactions=True
    )
    print(f"  Features: {features_df.shape} (was 1965x104 in iter_00)")
    ...
    print(f"  Feature count: {len(feature_cols)} (was 101 in iter_00)")
    ...
    # Replace NaN/inf from interactions
    ...
    # --- LSTM (unchanged) ---
    print("\n  Training LSTM (unchanged from iter_00)...")
